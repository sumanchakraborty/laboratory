  ...
  if (/TEST_NUM/) {
    ...
    if ($test_num != "") {
      ...
      if ($test_ref =~ /Setup Verification/) {
        ...
        $stripped_test_ref =~ /^$draft_name Section (.*) Page (.*) '(.*)'/;
        $exact_ref = "$1";
        if ($exact_ref == "") {
    ...
    if ($command == $tnum) {
      ...
      $tmp_section =~ /$draft_name Section (.*) Page /;
      if (($1 != "") && ($sections !~ /$1/)) {
        $sections .= ",$1";
      }
    }
...
for $i (0 .. $#nsorted) {
  ...
  if (!match_sec($draft_depth, $prev_sec, $curr_sec)) {
...
sub match_sec
{
  ...
  my @array1 = split(/\./, $info1);
  my @array2 = split(/\./, $info2);
